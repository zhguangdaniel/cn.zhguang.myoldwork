...
/**
 * 
 * IP包分析器，该程序来自于Jpcap官方网站上的开源例子:JpcapDumper。 略有修改
 * 
 * {@link http://netresearch.ics.uci.edu/kfujii/jpcap/doc/samples.html}
 * 
 * @author 修改人：张广
 */
public class IPAnalyzer extends IDPacketAnalyzer {
	private static final String[] valueNames = { "Version", "TOS: Priority",
			"TOS: Throughput", "TOS: Reliability", "Length", "Identification",
			"Fragment: Don't Fragment", "Fragment: More Fragment",
			"Fragment Offset", "Time To Live", "Protocol", "Source IP",
			"Destination IP", "Source Host Name", "Destination Host Name" };
	...
	public Object getValue(String valueName) {
		// if ((valueNames[13].equals(valueName) && values.get(valueName)
		// instanceof InetAddress)
		// || (valueNames[14].equals(valueName) && values.get(valueName)
		// instanceof InetAddress)) {
		//
		// InetAddress addr = (InetAddress) values.get(valueName);
		// /*
		// * if(JDCaptor.hostnameCache.containsKey(addr))
		// * values.put(valueName,JDCaptor.hostnameCache.get(addr)); else{
